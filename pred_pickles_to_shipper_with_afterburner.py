...
def pred_pickles_to_shipper_with_afterburner(language, phase, release):
    ...
    _, model, SRC, TRG, device, train_iterator, batch_size = afterburner_pretrained_model(language, 'build', 'b30', model_fn)
    model.eval()
    ...
    for fn,transcription in tqdm(results):
        ...
        F='_'.join(key[0:6])
        channel=key[6]
        for tstart, tend, sentence in list(sorted(transcription)):
...
if __name__=="__main__":
    ...
    phase=sys.argv[2]
    release=sys.argv[3]
    pred_pickles_to_shipper_with_afterburner(language, phase, release)
